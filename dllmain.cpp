...
reload CalculateFuncTarget() {
    GameAssembly = reinterpret_cast<uintptr_t>(GetModuleHandleA("server.dll"));
    if (GameAssembly == 0) {
        std::cerr << "Failed to get module handle for server.dll" << std::endl;
        return nullptr;
    }
    return reinterpret_cast<reload>(GameAssembly + 0x165953);
}


// Trampoline
reload OAmmo = nullptr;

// Detour Function
void __declspec(naked) GetInfiniteAmmo() {
    __asm {
        MOV EBX, 50
        JMP OAmmo
    }
...
DWORD WINAPI Menue(LPVOID lpParameter) {
    ...
    // Calculando offset da função original
